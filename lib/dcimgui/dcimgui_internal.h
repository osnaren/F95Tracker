...
// Helpers: Hashing
CIMGUI_API ImGuiID ImHashData(const void* data, size_t data_size);                                      // Implied seed = 0
CIMGUI_API ImGuiID ImHashDataEx(const void* data, size_t data_size, ImGuiID seed /* = 0 */);
CIMGUI_API ImGuiID ImHashStr(const char* data);                                                         // Implied data_size = 0, seed = 0
CIMGUI_API ImGuiID ImHashStrEx(const char* data, size_t data_size /* = 0 */, ImGuiID seed /* = 0 */);

// Helpers: Color Blending
CIMGUI_API ImU32 ImAlphaBlendColors(ImU32 col_a, ImU32 col_b);

// Helpers: Bit manipulation
CIMGUI_API bool         ImIsPowerOfTwo(int v);
CIMGUI_API bool         ImIsPowerOfTwoImU64(ImU64 v);
CIMGUI_API int          ImUpperPowerOfTwo(int v);
CIMGUI_API unsigned int ImCountSetBits(unsigned int v);

// Helpers: String
#define ImStrlen strlen
#define ImMemchr memchr
CIMGUI_API int         ImStricmp(const char* str1, const char* str2);                       // Case insensitive compare.
CIMGUI_API int         ImStrnicmp(const char* str1, const char* str2, size_t count);        // Case insensitive compare to a certain count.
CIMGUI_API void        ImStrncpy(char* dst, const char* src, size_t count);                 // Copy to a certain count and always zero terminate (strncpy doesn't).
CIMGUI_API char*       ImStrdup(const char* str);                                           // Duplicate a string.
CIMGUI_API char*       ImStrdupcpy(char* dst, size_t* p_dst_size, const char* str);         // Copy in provided buffer, recreate buffer if needed.
CIMGUI_API const char* ImStrchrRange(const char* str_begin, const char* str_end, char c);   // Find first occurrence of 'c' in string range.
CIMGUI_API const char* ImStreolRange(const char* str, const char* str_end);                 // End end-of-line
CIMGUI_API const char* ImStristr(const char* haystack, const char* haystack_end, const char* needle, const char* needle_end); // Find a substring in a string range.
CIMGUI_API void        ImStrTrimBlanks(char* str);                                          // Remove leading and trailing blanks from a buffer.
CIMGUI_API const char* ImStrSkipBlank(const char* str);                                     // Find first non-blank character.
CIMGUI_API int         ImStrlenW(const ImWchar* str);                                       // Computer string length (ImWchar string)
CIMGUI_API const char* ImStrbol(const char* buf_mid_line, const char* buf_begin);           // Find beginning-of-line
IM_MSVC_RUNTIME_CHECKS_OFF
CIMGUI_API char        ImToUpper(char c);
CIMGUI_API bool        ImCharIsBlankA(char c);
CIMGUI_API bool        ImCharIsBlankW(unsigned int c);
CIMGUI_API bool        ImCharIsXdigitA(char c);
IM_MSVC_RUNTIME_CHECKS_RESTORE

// Helpers: Formatting
CIMGUI_API int         ImFormatString(char* buf, size_t buf_size, const char* fmt, ...) IM_FMTARGS(3);
CIMGUI_API int         ImFormatStringV(char* buf, size_t buf_size, const char* fmt, va_list args) IM_FMTLIST(3);
CIMGUI_API void        ImFormatStringToTempBuffer(const char** out_buf, const char** out_buf_end, const char* fmt, ...) IM_FMTARGS(3);
CIMGUI_API void        ImFormatStringToTempBufferV(const char** out_buf, const char** out_buf_end, const char* fmt, va_list args) IM_FMTLIST(3);
CIMGUI_API const char* ImParseFormatFindStart(const char* format);
CIMGUI_API const char* ImParseFormatFindEnd(const char* format);
CIMGUI_API const char* ImParseFormatTrimDecorations(const char* format, char* buf, size_t buf_size);
CIMGUI_API void        ImParseFormatSanitizeForPrinting(const char* fmt_in, char* fmt_out, size_t fmt_out_size);
CIMGUI_API const char* ImParseFormatSanitizeForScanning(const char* fmt_in, char* fmt_out, size_t fmt_out_size);
CIMGUI_API int         ImParseFormatPrecision(const char* format, int default_value);

// Helpers: UTF-8 <> wchar conversions
CIMGUI_API const char* ImTextCharToUtf8(char out_buf[5], unsigned int c);                                                      // return out_buf
CIMGUI_API int         ImTextStrToUtf8(char* out_buf, int out_buf_size, const ImWchar* in_text, const ImWchar* in_text_end);   // return output UTF-8 bytes count
CIMGUI_API int         ImTextCharFromUtf8(unsigned int* out_char, const char* in_text, const char* in_text_end);               // read one character. return input UTF-8 bytes count
CIMGUI_API int         ImTextStrFromUtf8(ImWchar* out_buf, int out_buf_size, const char* in_text, const char* in_text_end);    // Implied in_remaining = NULL
CIMGUI_API int         ImTextStrFromUtf8Ex(ImWchar* out_buf, int out_buf_size, const char* in_text, const char* in_text_end, const char** in_remaining /* = NULL */); // return input UTF-8 bytes count
CIMGUI_API int         ImTextCountCharsFromUtf8(const char* in_text, const char* in_text_end);                                 // return number of UTF-8 code-points (NOT bytes count)
CIMGUI_API int         ImTextCountUtf8BytesFromChar(const char* in_text, const char* in_text_end);                             // return number of bytes to express one char in UTF-8
CIMGUI_API int         ImTextCountUtf8BytesFromStr(const ImWchar* in_text, const ImWchar* in_text_end);                        // return number of bytes to express string in UTF-8
CIMGUI_API const char* ImTextFindPreviousUtf8Codepoint(const char* in_text_start, const char* in_text_curr);                   // return previous UTF-8 code-point.
CIMGUI_API int         ImTextCountLines(const char* in_text, const char* in_text_end);                                         // return number of lines taken by text. trailing carriage return doesn't count as an extra line.

// Helpers: File System
#ifdef IMGUI_DISABLE_FILE_FUNCTIONS
#define IMGUI_DISABLE_DEFAULT_FILE_FUNCTIONS
typedef void* ImFileHandle;
CIMGUI_API ImFileHandle ImFileOpen(const char*, const char*);
CIMGUI_API bool         ImFileClose(ImFileHandle);
CIMGUI_API ImU64        ImFileGetSize(ImFileHandle);
CIMGUI_API ImU64        ImFileRead(void*, ImU64, ImU64, ImFileHandle);
CIMGUI_API ImU64        ImFileWrite(const void*, ImU64, ImU64, ImFileHandle);
#endif // #ifdef IMGUI_DISABLE_FILE_FUNCTIONS
#ifndef IMGUI_DISABLE_DEFAULT_FILE_FUNCTIONS
typedef FILE* ImFileHandle;
CIMGUI_API ImFileHandle ImFileOpen(const char* filename, const char* mode);
CIMGUI_API bool         ImFileClose(ImFileHandle file);
CIMGUI_API ImU64        ImFileGetSize(ImFileHandle file);
CIMGUI_API ImU64        ImFileRead(void* data, ImU64 size, ImU64 count, ImFileHandle file);
CIMGUI_API ImU64        ImFileWrite(const void* data, ImU64 size, ImU64 count, ImFileHandle file);
#else
#define IMGUI_DISABLE_TTY_FUNCTIONS  // Can't use stdout, fflush if we are not using default file functions
#endif // #ifndef IMGUI_DISABLE_DEFAULT_FILE_FUNCTIONS
CIMGUI_API void* ImFileLoadToMemory(const char* filename, const char* mode);                                                                      // Implied out_file_size = NULL, padding_bytes = 0
CIMGUI_API void* ImFileLoadToMemoryEx(const char* filename, const char* mode, size_t* out_file_size /* = NULL */, int padding_bytes /* = 0 */);
...
CIMGUI_API float  ImPow(float x, float y);           // DragBehaviorT/SliderBehaviorT uses ImPow with either float/double and need the precision
CIMGUI_API double ImPowDouble(double x, double y);
CIMGUI_API float  ImLog(float x);                    // DragBehaviorT/SliderBehaviorT uses ImLog with either float/double and need the precision
CIMGUI_API double ImLogDouble(double x);
CIMGUI_API int    ImAbs(int x);
CIMGUI_API float  ImAbsFloat(float x);
CIMGUI_API double ImAbsDouble(double x);
CIMGUI_API float  ImSign(float x);                   // Sign operator - returns -1, 0 or 1 based on sign of argument
CIMGUI_API double ImSignDouble(double x);
#ifdef IMGUI_ENABLE_SSE
CIMGUI_API float ImRsqrt(float x);
#else
CIMGUI_API float ImRsqrtFloat(float x);
#endif // #ifdef IMGUI_ENABLE_SSE
CIMGUI_API double ImRsqrtDouble(double x);
#endif // #ifndef IMGUI_DISABLE_DEFAULT_MATH_FUNCTIONS
// - Misc maths helpers
CIMGUI_API ImVec2 ImMin(ImVec2 lhs, ImVec2 rhs);
CIMGUI_API ImVec2 ImMax(ImVec2 lhs, ImVec2 rhs);
CIMGUI_API ImVec2 ImClamp(ImVec2 v, ImVec2 mn, ImVec2 mx);
CIMGUI_API ImVec2 ImLerp(ImVec2 a, ImVec2 b, float t);
CIMGUI_API ImVec2 ImLerpImVec2(ImVec2 a, ImVec2 b, ImVec2 t);
CIMGUI_API ImVec4 ImLerpImVec4(ImVec4 a, ImVec4 b, float t);
CIMGUI_API float  ImSaturate(float f);
CIMGUI_API float  ImLengthSqr(ImVec2 lhs);
CIMGUI_API float  ImLengthSqrImVec4(ImVec4 lhs);
CIMGUI_API float  ImInvLength(ImVec2 lhs, float fail_value);
CIMGUI_API float  ImTrunc(float f);
CIMGUI_API ImVec2 ImTruncImVec2(ImVec2 v);
CIMGUI_API float  ImFloor(float f);                                                      // Decent replacement for floorf()
CIMGUI_API ImVec2 ImFloorImVec2(ImVec2 v);
CIMGUI_API int    ImModPositive(int a, int b);
CIMGUI_API float  ImDot(ImVec2 a, ImVec2 b);
CIMGUI_API ImVec2 ImRotate(ImVec2 v, float cos_a, float sin_a);
CIMGUI_API float  ImLinearSweep(float current, float target, float speed);
CIMGUI_API float  ImLinearRemapClamp(float s0, float s1, float d0, float d1, float x);
CIMGUI_API ImVec2 ImMul(ImVec2 lhs, ImVec2 rhs);
CIMGUI_API bool   ImIsFloatAboveGuaranteedIntegerPrecision(float f);
CIMGUI_API float  ImExponentialMovingAverage(float avg, float sample, int n);
IM_MSVC_RUNTIME_CHECKS_RESTORE

// Helpers: Geometry
CIMGUI_API ImVec2 ImBezierCubicCalc(ImVec2 p1, ImVec2 p2, ImVec2 p3, ImVec2 p4, float t);
CIMGUI_API ImVec2 ImBezierCubicClosestPoint(ImVec2 p1, ImVec2 p2, ImVec2 p3, ImVec2 p4, ImVec2 p, int num_segments);               // For curves with explicit number of segments
CIMGUI_API ImVec2 ImBezierCubicClosestPointCasteljau(ImVec2 p1, ImVec2 p2, ImVec2 p3, ImVec2 p4, ImVec2 p, float tess_tol);        // For auto-tessellated curves you can use tess_tol = style.CurveTessellationTol
CIMGUI_API ImVec2 ImBezierQuadraticCalc(ImVec2 p1, ImVec2 p2, ImVec2 p3, float t);
CIMGUI_API ImVec2 ImLineClosestPoint(ImVec2 a, ImVec2 b, ImVec2 p);
CIMGUI_API bool   ImTriangleContainsPoint(ImVec2 a, ImVec2 b, ImVec2 c, ImVec2 p);
CIMGUI_API ImVec2 ImTriangleClosestPoint(ImVec2 a, ImVec2 b, ImVec2 c, ImVec2 p);
CIMGUI_API void   ImTriangleBarycentricCoords(ImVec2 a, ImVec2 b, ImVec2 c, ImVec2 p, float* out_u, float* out_v, float* out_w);
CIMGUI_API float  ImTriangleArea(ImVec2 a, ImVec2 b, ImVec2 c);
CIMGUI_API bool   ImTriangleIsClockwise(ImVec2 a, ImVec2 b, ImVec2 c);
...
CIMGUI_API size_t ImBitArrayGetStorageSizeInBytes(int bitcount);
CIMGUI_API void   ImBitArrayClearAllBits(ImU32* arr, int bitcount);
CIMGUI_API bool   ImBitArrayTestBit(const ImU32* arr, int n);
CIMGUI_API void   ImBitArrayClearBit(ImU32* arr, int n);
CIMGUI_API void   ImBitArraySetBit(ImU32* arr, int n);
CIMGUI_API void   ImBitArraySetBitRange(ImU32* arr, int n, int n2);                                                  // Works on range [n..n2)
...
// Helper: ImGuiStorage
CIMGUI_API ImGuiStoragePair* ImLowerBound(ImGuiStoragePair* in_begin, ImGuiStoragePair* in_end, ImGuiID key);
...
CIMGUI_API const ImFontBuilderIO* ImFontAtlasGetBuilderForStbTruetype(void);
#endif // #ifdef IMGUI_ENABLE_STB_TRUETYPE
CIMGUI_API void ImFontAtlasUpdateSourcesPointers(ImFontAtlas* atlas);
CIMGUI_API void ImFontAtlasBuildInit(ImFontAtlas* atlas);
CIMGUI_API void ImFontAtlasBuildSetupFont(ImFontAtlas* atlas, ImFont* font, ImFontConfig* src, float ascent, float descent);
CIMGUI_API void ImFontAtlasBuildPackCustomRects(ImFontAtlas* atlas, void* stbrp_context_opaque);
CIMGUI_API void ImFontAtlasBuildFinish(ImFontAtlas* atlas);
CIMGUI_API void ImFontAtlasBuildRender8bppRectFromString(ImFontAtlas* atlas, int x, int y, int w, int h, const char* in_str, char in_marker_char, unsigned char in_marker_pixel_value);
CIMGUI_API void ImFontAtlasBuildRender32bppRectFromString(ImFontAtlas* atlas, int x, int y, int w, int h, const char* in_str, char in_marker_char, unsigned int in_marker_pixel_value);
CIMGUI_API void ImFontAtlasBuildMultiplyCalcLookupTable(unsigned char out_table[256], float in_multiply_factor);
CIMGUI_API void ImFontAtlasBuildMultiplyRectAlpha8(const unsigned char table[256], unsigned char* pixels, int x, int y, int w, int h, int stride);
CIMGUI_API void ImFontAtlasBuildGetOversampleFactors(const ImFontConfig* src, int* out_oversample_h, int* out_oversample_v);

CIMGUI_API bool ImFontAtlasGetMouseCursorTexData(ImFontAtlas* atlas, ImGuiMouseCursor cursor_type, ImVec2* out_offset, ImVec2* out_size, ImVec2 out_uv_border[2], ImVec2 out_uv_fill[2]);
...
#ifdef IMGUI_ENABLE_TEST_ENGINE
extern
CIMGUI_API void        ImGuiTestEngineHook_ItemAdd(ImGuiContext* ctx, ImGuiID id, ImRect bb, const ImGuiLastItemData* item_data); // item_data may be NULL
extern
CIMGUI_API void        ImGuiTestEngineHook_ItemInfo(ImGuiContext* ctx, ImGuiID id, const char* label, ImGuiItemStatusFlags flags);
extern
CIMGUI_API void        ImGuiTestEngineHook_Log(ImGuiContext* ctx, const char* fmt, ...);
extern
CIMGUI_API const char* ImGuiTestEngine_FindItemDebugLabel(ImGuiContext* ctx, ImGuiID id);
